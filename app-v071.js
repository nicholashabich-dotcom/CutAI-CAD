...
const svg=$("cad"),geometry=$("geometry"),dimensions=$("dimensions"),camMarkers=$("camMarkers"),simLayer=$("simLayer"),preview=$("preview"),snapLayer=$("snapLayer"),statusEl=$("status"),coordsEl=$("coords"),properties=$("properties"),geometryFields=$("geometryFields"),propTitle=$("propTitle"),edgeInfo=$("edgeInfo"),contourList=$("contourList"),projectSummary=$("projectSummary");
...
function newProject(){
 return{schema:"cutai-cad/0.7.1",units:"mm",name:"CutAI Projekt",machineProfile:profileSnapshot(BUILTIN_PROFILE),
...
const state={
 ...
 viewBox:{x:0,y:0,w:1200,h:800},pointers:new Map(),pan:null,pinch:null,drag:null,drawing:null,arcStage:null,startPointMode:false,simulation:false
};
...
function normalizeProject(p){
 if((p?.schema==="cutai-cad/0.7"||p?.schema==="cutai-cad/0.7.1")&&Array.isArray(p.contours)){
  p=deep(p);const def=profileSnapshot(BUILTIN_PROFILE);p.schema="cutai-cad/0.7.1";p.machineProfile={...def,...(p.machineProfile||{})};
...
function setTool(t){state.tool=t;state.drawing=null;state.arcStage=null;state.startPointMode=false;preview.replaceChildren();document.querySelectorAll(".tool").forEach(b=>b.classList.toggle("active",b.dataset.tool===t));status(t==="arc"?"Bogen: Mittelpunkt antippen":`${document.querySelector(`[data-tool="${t}"] small`)?.textContent||t}`)}
...
function contourEl(c){let el;if(c.geometry.kind==="circle"){el=document.createElementNS(NS,"circle");el.setAttribute("cx",c.geometry.cx);el.setAttribute("cy",c.geometry.cy);el.setAttribute("r",c.geometry.r)}else{el=document.createElementNS(NS,"path");let d="";const segs=c.geometry.segments||[];if(segs.length){const p=C.segStart(segs[0]);d=`M ${p.x} ${p.y}`;for(const s of segs)d+=s.type==="line"?` L ${s.x2} ${s.y2}`:` A ${s.r} ${s.r} 0 ${C.arcSweep(s)>Math.PI?1:0} ${s.cw?1:0} ${s.ex} ${s.ey}`;if(c.closed)d+=" Z"}el.setAttribute("d",d)}el.setAttribute("class",`shape ${c.closed?"":"open"} ${c.cam?.tool?.role==="mark"?"mark":""} ${state.selectedIds.includes(c.id)?"selected":""}`);el.dataset.id=c.id;return el}
...
function renderList(){contourList.replaceChildren();const ordered=[...contours()].sort((a,b)=>(a.cam.order||0)-(b.cam.order||0));for(const c of ordered){const d=document.createElement("div");d.className=`contour-item ${state.selectedIds.includes(c.id)?"selected":""}`;const bevel=c.cam.bevel?.enabled?` · ${c.cam.bevel.profileType}-Fase`:"";d.innerHTML=`<strong>${escapeHtml(c.name)}</strong><div class="contour-meta">#${c.cam.order} · ${escapeHtml(c.cam.tool.code)} · ${resolvedType(c)}${bevel}</div><div class="contour-meta ${c.closed?"":"contour-bad"}">${c.closed?"geschlossen":"offen"} · Layer ${escapeHtml(c.sourceLayer||"CUTAI")}</div>`;d.onclick=()=>{if(state.multi){toggleSelection(c.id)}else{state.selectedIds=[c.id];render();showProperties()}};contourList.appendChild(d)}
 projectSummary.textContent=`${state.project.name} · ${state.project.material.name} ${fmt(state.project.material.thicknessMm)} mm · ${state.project.machineProfile.name}`;
}
function escapeHtml(s){return String(s).replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[m]))}
function render(){geometry.replaceChildren();dimensions.replaceChildren();camMarkers.replaceChildren();simLayer.replaceChildren();for(const c of contours()){geometry.appendChild(contourEl(c));if(state.selectedIds.includes(c.id))renderDimensions(c);renderMarkers(c)}if(state.simulation)renderSimulation();renderList();setViewBox()}
...
function exportCamPlan(){const report=C.validateProject(state.project),ops=[...contours()].sort((a,b)=>a.cam.order-b.cam.order).map(c=>({id:c.id,name:c.name,order:c.cam.order,tool:c.cam.tool,contourType:resolvedType(c),direction:c.cam.direction,startPoint:c.cam.startPoint,kerfMm:c.cam.kerfMm,kerfComp:c.cam.kerfComp,leadIn:c.cam.leadIn,leadOut:c.cam.leadOut,bevel:c.cam.bevel,sourceLayer:c.sourceLayer,sourceRule:c.cam.sourceRule||null,lengthMm:C.contourLength(c),geometry:deep(c.geometry)}));const plan={schema:"cutai-cam-plan/0.7.1",units:"mm",project:state.project.name,machineProfile:state.project.machineProfile,material:state.project.material,postprocessorTarget:state.project.machineProfile.postprocessorTarget||"neutral",referenceBuild:R.buildReference||null,note:"Neutraler CAM-Plan. Das Postprozessor-Ziel ist nur Metadatum; es werden keine Maschinen-M-Codes und keine direkte Maschinensteuerung erzeugt.",validation:report,simulation:simulationMetrics(),operations:ops};download("cutai-cam-plan-v071.json",JSON.stringify(plan,null,2),"application/json");status("Neutraler CAM-Plan v0.7.1 exportiert")}
...
function showReference(){
 const b=R.buildReference||{},p=BUILTIN_PROFILE,mods=R.sourceModules||[],pp=R.postprocessorReferences||[];
...
$("newBtn").onclick=()=>{snapshot();state.project=newProject();state.selectedIds=[];state.simulation=false;hideProperties();fillToolSelect();render();status("Neues Projekt v0.7.1")};
$("saveBtn").onclick=()=>download("cutai-project-v071.json",JSON.stringify(state.project,null,2),"application/json");
...
$("joinBtn").onclick=joinContours;$("autoCamBtn").onclick=autoCam;$("validateBtn").onclick=validate;$("simBtn").onclick=toggleSimulation;$("referenceBtn").onclick=showReference;
$("dxfBtn").onclick=()=>{if(!contours().length)return status("Keine Konturen");download("cutai-export-v071-r12.dxf",C.makeDxf(contours(),layerName),"application/dxf");status("DXF R12 exportiert")};
...
try{const auto=localStorage.getItem("cutai-v071-autosave")||localStorage.getItem("cutai-v07-autosave");if(auto)state.project=normalizeProject(JSON.parse(auto))}catch{}
setInterval(()=>{try{localStorage.setItem("cutai-v071-autosave",JSON.stringify(state.project))}catch{}},3000);
fillToolSelect();render();status("Bereit · v0.7.1 · ASPER-Referenz geladen · kein Maschinen-NC")
})();
