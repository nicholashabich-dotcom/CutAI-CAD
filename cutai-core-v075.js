  ...
  function validateProject(project){
    ...
    for(const c of project.contours||[]){const label=c.name||c.id||"Kontur";const role=c.cam?.tool?.role||"cut";
      ...
      if(c.cam?.bevel?.enabled){
        ...
        for(const [i,f] of (b.faces||[]).entries()){if(Math.abs(Number(f.angleDeg)||0)>limit+EPS)errors.push(`${label}: Fasenfläche ${i+1} überschreitet ${limit}°.`)}
        if(a1>limit+EPS||a2>limit+EPS)errors.push(`${label}: Alfa1/Alfa2 überschreitet ${limit}°.`);
        if(["K","X"].includes(String(b.profileType))&&tp+tn>th+EPS)errors.push(`${label}: TP + TN (${round(tp+tn,3)} mm) ist größer als Materialstärke (${round(th,3)} mm).`);
        if(String(b.profileType).startsWith("Y_")&&tn>th+EPS)errors.push(`${label}: TN (${round(tn,3)} mm) ist größer als Materialstärke (${round(th,3)} mm).`);
